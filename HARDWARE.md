...
| Any card, Pentium OverDrive 83 under EMM386: `RDTSC` from real-mode code hard-hangs the machine (vcctrl rig, 2026-09-03) | Real, reproduced (dinspect twice, HWiNFO for DOS once), mechanism not identified -- worked around, not fixed | The first `RDTSC` executed by a real-mode (V86-task) program under MS-DOS 6.22 `EMM386 NOEMS` never returns: interrupts still serviced (keyboard LEDs respond), no forward progress, Ctrl-Alt-Del dead, power cycle required. `CPUID` is fine. The OverDrive is the only CPU in this matrix with a TSC, so 486-class runs can never surface it. DJGPP protected-mode code is a different execution context (ring-3 DPMI client of CWSDPMI) and has been observed NOT to hang on the same rig/CPU/EMM386 line: doskutsu's 2026-08-13 r2-POD83 QA matrix ran `shared/patches/sdl3-dos/0039`'s RDTSC audio-IRQ timer for whole sessions. Real-mode helpers must use dinspect's `SMSW`/CR0.PE check before `RDTSC`; DJGPP code cannot use that check (PE is always set) and CWSDPMI hardcodes the DPMI 0400h V86 flag, so the shared layer keeps RDTSC on by default and ships a runtime killswitch, `SDL_HINT_DOS_AUDIO_TIMER_RDTSC=0` (`0138`), instead of an automatic guard. | `docs/timing.md` (RDTSC section), `shared/patches/sdl3-dos/0138` |
