...
**Status:** adopted by this hub. Originally written inside doskutsu
("kept internal during incubation... will move to its own repository
once a second project adopts it" -- that document's own stated graduation
condition), and moved here once sdl-dos-ports became that second
project. Reproduced here near-verbatim (relicensed MIT for this hub, with
the operator's approval, matching how `shared/include/runmanifest.h` was
handled -- original work of the same author/project, no third-party GPL
dependency; doskutsu's own copy is untouched) because the text is already
written to be project-agnostic: nothing in the normative sections below
assumes any particular project. Project-specific requirements belong in a
*profile* (sec. 12), not in this document.

**How this fits with the rest of the hub:** this is the normative
standard. `shared/skills/dos-hardware-validation/references/
harness-invariants.md` and `dos-realhw-verification` are the *practice*
layer -- how this standard's rules apply concretely to a vcctrl-driven
DOS rig, with this hub's own worked examples. Read this document once for
the underlying contract; read those skills for the day-to-day mechanics
of actually following it here.
...
COMMAND.COM / DJGPP -- this hub's own current version of this table lives
in `docs/dos-scripting.md`:
...
exit carrying build identity and headline metrics. **MUST for L1.** This
hub's `shared/include/runmanifest.h` is a ready-to-use implementation --
see `shared/skills/dos-hardware-validation/references/runmanifest-log.md`.
...
This hub's ports are each a separate adopter: a profile lives with
whichever project declares it (this hub itself, or a specific port), not
in this document.
