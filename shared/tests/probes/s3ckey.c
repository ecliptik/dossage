/*
 * s3ckey.c -- S3 ViRGE/DX K4 COLORKEY-BLT throughput de-risk probe (S3-2 #10a).
 *
 * Built on the PROVEN s3blt v6 harness (copied; CR53 new-MMIO enable, bit13 idle
 * + busy-then-idle bracket, ROP<<17, RWIDTH_HEIGHT=(w-1)<<16|RAW h -- all
 * sdl-engine source-verified on real HW). K4 ADDS source-transparency: a colorkey
 * BLT (CMD_SET |= TP bit9 = 0x07980221) with the key in SRC_BG_CLR/SRC_FG_CLR
 * (register-ladder, sdl-engine consult; bit9 named MIX_MONO_TRANSP so the
 * dst-readback oracle CONFIRMS it colorkeys a FORMAT8 color BLT). It measures the
 * colorkey BLT vs the opaque BLT (A/B) vs the CPU colorkey blit at the sprite-
 * tile geoms (16x16 FIRST = the gate tile), and emits a K4 GATE_16x16 verdict.
 *
 * THE K4 QUESTION: v6 opaque BLT was ~3.7x the CPU at 16x16. Cave Story sprites
 * need COLORKEY (skip transparent px). Does the colorkey BLT KEEP that edge, or
 * does per-pixel source transparency tank it? GREEN_CK_KEEPS_EDGE (ck/cpu>=3.0) /
 * AMBER_CK_REDUCED_EDGE (beats CPU, edge shrunk) / RED_CK_LOSES_TO_CPU /
 * CK_UNVERIF (TP couldn't colorkey -- a finding, not a bug). verify-before-trust:
 * dst KEEPS sentinel where src==key + COPIES elsewhere -> UNVERIF never a false
 * MB/s. TRUNCATION FIX (v6 died at the 320x240 fill): tile geoms only, 16x16
 * first + fsync-per-line so the gate result survives any late-geom hang; the K2
 * solid-fill kernel is retained-but-unused (no big-write hang risk).
 *
 * --- inherited s3blt lineage (the harness this is built on) ------------------
 ...
 *
 * Output: S3CKEY.LOG (CWD), fallback C:\S3CKEY.LOG. fsync per line.
 *
 ...
 *
 * 8.3 DOS filename: S3CKEY.EXE (5.3) -- fits. Log S3CKEY.LOG.
 *
 * License: MIT.
 */
...
/* ============================================================ */
/* Logging -- fopen-direct S3CKEY.LOG + stdout mirror, fsync/line */
/* ============================================================ */
...
{
    g_log = fopen("S3CKEY.LOG", "w");
    if (!g_log) g_log = fopen("C:\\S3CKEY.LOG", "w");
}
...
/* 8bpp COLORKEY (source-transparency) BLT (K4, task #10a). Opaque SRCCOPY | TP
 * (bit9, 86Box CMD_SET_TP) -> source pixels EQUAL to the key (programmed into
 * the key-reg) are skipped, dst keeps its value. sdl-engine consult: key lives
 * in SRC_BG_CLR (rung A) or SRC_FG_CLR (rung B); bit9 is named MIX_MONO_TRANSP
 * in regs3v.h, so the readback oracle + the register ladder confirm it really
 * colorkeys a FORMAT8 color BLT (not just mono-source expansion). */
#define CMD_TP_BIT       0x00000200UL          /* CMD_SET bit9 = source transparency */
#define CMD_BLT_CK_8BPP  (CMD_BLT_8BPP | CMD_TP_BIT)
#define CK_KEY           0xFF                   /* source colorkey == CPU_COLORKEY (A/B parity) */
...
/* Colorkey screen-to-screen BLT: like s3_blt_copy but enables source
 * transparency (CMD_TP_BIT) + programs the colorkey into `key_reg` (replicated
 * to all 4 bytes -- the engine may compare per-byte across a dword). Source
 * pixels EQUAL to `key` are skipped (dst keeps its value). Returns 0 ok, -1
 * engine stuck. The K4 throughput question rides on this path. */
static int s3_blt_copy_ck(uint32_t dst_off, uint32_t src_off, int w, int h,
                          int w_m1, uint8_t key, uint32_t key_reg)
{
    if (s3_wait_idle() != 0) return -1;
    uint16_t ww = (uint16_t)(w_m1 ? (w - 1) : w);
    uint16_t hh = (uint16_t)h;                  /* RAW height (ViRGE quirk)   */
    uint32_t stride = (uint32_t)w;
    uint32_t key4 = (uint32_t)key;
    key4 |= key4 << 8; key4 |= key4 << 16;      /* replicate key to 4 bytes   */
    ...
    mmio_w32(R_CLIP_T_B,      (0u << 16) | 0x07FFu);
    mmio_w32(R_DEST_SRC_STR,  (stride << 16) | stride);
    mmio_w32(key_reg,         key4);            /* source colorkey            */
    mmio_w32(R_CMD_SET,       CMD_BLT_CK_8BPP);
    mmio_w32(R_RWIDTH_HEIGHT, ((uint32_t)ww << 16) | hh);
    mmio_w32(R_RSRC_XY,       0u);
    mmio_w32(R_RDEST_XY,      0u);              /* last write -> autoexecute   */
    if (s3_wait_done() != 0) return -1;
    return 0;
}
...
/* K4 colorkey probe: TILE geometries only. The 320x240 + 16x256 geoms (and
 * their large FILL kernel) are DROPPED -- the v6 opaque run TRUNCATED at the
 * 320x240 fill (largest write), and K4 is judged at the sprite-tile sizes, not
 * a backdrop/strip. 16x16 is FIRST so the gate tile's result is logged + fsync'd
 * before any later geom can hang (refinement 1). */
#define GEOM_COUNT 3
static const struct { const char *name; int w; int h; int is_trap; } g_geom[GEOM_COUNT] = {
    { "16x16",   16,  16,  0 },   /* THE decision tile  -> 256 B  (gate, measured FIRST) */
...
static void plaus_check(const char *what, const char *geom, double mbs)
{
    if (mbs < PLAUS_LO_MBS || mbs > PLAUS_HI_MBS) {
        g_warn++;
        slog("S3CKEY-WARN %s geom=%s = %.2f MB/s OUTSIDE plausible window "
...
/* ---- K4: colorkey BLT verify (key-register ladder) + throughput ---- */

/* Colorkey test src byte: even offset = CK_KEY (transparent), odd = non-key. */
static inline uint8_t ck_src_byte(uint32_t i)
{
    return (i & 1) ? (uint8_t)(0x40 + (i & 0x3F)) : CK_KEY;
}

/* Verify one colorkey BLT at geom gi, width-conv `conv`, key in `key_reg`.
 * Oracle: dst KEEPS the 0xCC sentinel where src==CK_KEY + COPIES src elsewhere.
 * Returns 1 byte-correct, 0 otherwise (stuck, or transparency not applied). */
static int ck_verify_one(int gi, int conv, uint32_t key_reg)
{
    int w = g_geom[gi].w, h = g_geom[gi].h;
    uint32_t bytes = (uint32_t)w * (uint32_t)h;
    for (uint32_t i = 0; i < bytes; i++) lfb_wb(VRAM_SRC_OFF + i, ck_src_byte(i));
    lfb_fill(VRAM_DST_OFF, 0xCC, bytes);
    if (s3_blt_copy_ck(VRAM_DST_OFF, VRAM_SRC_OFF, w, h, conv, CK_KEY, key_reg) != 0)
        return 0;
    for (uint32_t i = 0; i < bytes; i++) {
        uint8_t src  = ck_src_byte(i);
        uint8_t want = (src == CK_KEY) ? 0xCC : src;   /* key -> keep sentinel */
        if (lfb_rb(VRAM_DST_OFF + i) != want) return 0;
    }
    return 1;
}

/* Key-register ladder: SRC_BG_CLR (rung A) then SRC_FG_CLR (rung B). Returns the
 * verified key_reg, or 0 if neither colorkeys correctly (TP bit9 may be mono-
 * source-only on a FORMAT8 color BLT -- itself the K4 finding, not a bug). */
static uint32_t ck_resolve_keyreg(int gi, int conv)
{
    if (ck_verify_one(gi, conv, R_SRC_BG_CLR)) {
        slog("  ck-verify geom=%s: OK key_reg=SRC_BG_CLR(0x%04X) rungA", g_geom[gi].name, R_SRC_BG_CLR);
        return R_SRC_BG_CLR;
    }
    if (ck_verify_one(gi, conv, R_SRC_FG_CLR)) {
        slog("  ck-verify geom=%s: OK key_reg=SRC_FG_CLR(0x%04X) rungB", g_geom[gi].name, R_SRC_FG_CLR);
        return R_SRC_FG_CLR;
    }
    slog("  ck-verify geom=%s: FAIL both rungs (TP=bit9 may be mono-only / key-reg "
         "differs) -- colorkey UNVERIF, never a false MB/s", g_geom[gi].name);
    return 0;
}

/* Colorkey BLT throughput (median MB/s) at geom gi, conv, resolved key_reg. */
static double ck_throughput(int gi, int conv, uint32_t key_reg)
{
    int w = g_geom[gi].w, h = g_geom[gi].h;
    double bytes = (double)w * (double)h;
    double samp[ROUNDS]; long N;
    {
        uint64_t c0 = rdtsc();
        for (int i = 0; i < CALIB_ITERS; i++)
            s3_blt_copy_ck(VRAM_DST_OFF, VRAM_SRC_OFF, w, h, conv, CK_KEY, key_reg);
        double per = cycles_to_us(rdtsc() - c0) / (double)CALIB_ITERS;
        if (per <= 0.0) per = 1.0;
        N = (long)((BATCH_SECS * 1e6) / per) + 1; if (N < 8) N = 8;
    }
    for (int r = 0; r < ROUNDS; r++) {
        uint64_t c0 = rdtsc();
        for (long i = 0; i < N; i++)
...
/* ============================================================ */
/* K2 -- solid rect-fill (retained from the s3blt v6 lineage;     */
/* UNUSED in the colorkey probe -- the fill kernel was dropped per */
/* the v6 320x240 truncation fix, kept here + in s3_rect_fill for  */
/* reference / a future iter). __attribute__((unused)) keeps the   */
/* -Wall build clean without deleting the proven code.             */
/* ============================================================ */

static int __attribute__((unused)) k2_fill_verify(int gi, int conv)
{
    int w = g_geom[gi].w, h = g_geom[gi].h;
    uint32_t bytes = (uint32_t)w * (uint32_t)h;
    lfb_fill(VRAM_DST_OFF, 0xCC, bytes);
...
static void __attribute__((unused)) k2_fill_throughput(int gi, int conv, double *med)
{
    int w = g_geom[gi].w, h = g_geom[gi].h;
    double bytes = (double)w * (double)h;
    double samp[ROUNDS];
...
static void s3_status_forensics(void)
{
    ...
    if ((after & S3D_IDLE_BIT) == 0) {
        g_warn++;
        slog("  S3CKEY-WARN engine did NOT return to idle (bit 0x2000) within the "
        ...
        g_warn++;
        slog("  S3CKEY-WARN SUBSYS_STAT never showed busy across a kick -- the "
             "engine may not have run; throughput timings suspect. flush-instr: "
             "cross-check vs MEMBW LFB rate.");
    } else {
        slog("  S3CKEY-OK idle bit toggled correctly across the kick "
 ...
 * silicon). The real g2k+ViRGE iter runs S3CKEY.EXE with NO args (auto-detect).
...
int main(int argc, char **argv)
{
    ...
    open_log();
    slog("=== S3CKEY -- S3 ViRGE/DX 2D BitBLT engine probe (campaign P3) ===");
    slog("S3CKEY-BEGIN");
    slog("version: s3ckey-v1 (K4 colorkey BLT throughput, task #10a; built on the "
         "s3blt v6 harness: CR53|=0x08 new-MMIO, idle=bit13 0x2000 + busy-then-idle "
         "bracket, ROP<<17 [CMD_SET opaque 0x07980021], RWIDTH_HEIGHT=(w-1)<<16|RAW h. "
         "K4 adds: source transparency CMD_TP=bit9 [colorkey CMD_SET 0x07980221], "
         "key in SRC_BG_CLR/SRC_FG_CLR register-ladder, dst-readback colorkey oracle; "
         "16x16 measured FIRST, K2 fill DROPPED [v6 truncation fix].)");
    slog("plan: K4 colorkey de-risk -- does colorkey keep v6's opaque ~3.7x@16x16?");
    ...
    if (g_us_per_cycle <= 0.0) {
        ...
        slog("[s3ckey SUITE_DONE verdict=ABORT_NO_TIMER]");
        slog("S3CKEY-DONE");
    ...
    if (!is_virge && !g_force) {
        slog("S3CKEY-WARN no S3 ViRGE signature (PCI vendor != 0x5333 AND "
             "CR2D != 0x8A) -- skipping all 2D kernels (chip-specific probe; "
             "expected under DOSBox-X). Pass FORCE to exercise the mapping path.");
        slog("[s3ckey SUITE_DONE verdict=SKIP_NOT_S3_VIRGE]");
        slog("S3CKEY-DONE");
        if (g_log) fclose(g_log);
        return 0;
    }
    if (!is_virge && g_force) {
        slog("S3CKEY-FORCE proceeding past detection (CR2D=0x%02X != 0x8A) for "
    ...
    if (mode == 0) {
        ...
        slog("[s3ckey SUITE_DONE verdict=ABORT_NO_LFB_MODE]");
        slog("S3CKEY-DONE");
        if (g_log) fclose(g_log);
        return 2;
    }
    ...
    if (VRAM_SRC_OFF + max_bytes > total_vram) {
        ...
        slog("[s3ckey SUITE_DONE verdict=ABORT_VRAM_TOO_SMALL]");
        slog("S3CKEY-DONE");
        if (g_log) fclose(g_log);
        return 2;
    }
    ...
    if (vbe_set_mode_lfb(mode) != 0) {
        ...
        slog("[s3ckey SUITE_DONE verdict=ABORT_SETMODE]");
        slog("S3CKEY-DONE");
        if (g_log) fclose(g_log);
        return 2;
    }
    ...
    if (!near_ok) {
        ...
        slog("[s3ckey SUITE_DONE verdict=ABORT_MAP_FAILED]");
        slog("S3CKEY-DONE");
        if (g_log) fclose(g_log);
        return 2;
    }
    ...
    double ck16 = -1.0; int ck16_ok = 0;          /* K4 colorkey BLT @16x16 */
    ...
    slog("[s3ckey SUITE_BEGIN chip=0x%02X%02X mode=0x%04X %ux%u]",
    ...
    for (int gi = 0; gi < GEOM_COUNT; gi++) {
        ...
        if (conv >= 0) {
            ...
            slog("S3CKEY-RAW geom=%-7s blt conv=%-7s iters=%-6ld MB/s "
        ...
        if (cpu_med > 0.0) {
            slog("S3CKEY-RAW geom=%-7s cpu colorkey-blit MB/s med=%7.2f",
                 g_geom[gi].name, cpu_med);
            plaus_check("cpu", g_geom[gi].name, cpu_med);
        }

        /* K4 colorkey BLT: resolve the key-register (ladder) at the opaque
         * width-conv, then time it. The readback oracle gates it -> UNVERIF
         * never a false MB/s. (K2 solid-fill DROPPED here: not colorkey-relevant
         * + it was the v6 320x240 truncation culprit.) */
        double ck_med = 0; uint32_t ck_reg = 0; int ck_ok = 0;
        if (conv >= 0) {
            ck_reg = ck_resolve_keyreg(gi, conv);
            if (ck_reg) {
                ck_med = ck_throughput(gi, conv, ck_reg);
                ck_ok = 1;
                slog("S3CKEY-RAW geom=%-7s colorkey-blt MB/s med=%7.2f key_reg=0x%04X",
                     g_geom[gi].name, ck_med, (unsigned)ck_reg);
                plaus_check("ck", g_geom[gi].name, ck_med);
            }
        }

        /* contract STAT line -- flush-instr keys the K4 gate off it. ck_MBps =
         * ViRGE colorkey BLT; blt_MBps = opaque BLT (A/B); cpu_MBps = CPU
         * colorkey blit (the bar colorkey must beat). */
        if (conv >= 0 && cpu_med > 0.0) {
            slog("[s3ckey geom=%-7s ck_MBps=%s%.1f%s blt_MBps=%.1f cpu_MBps=%.1f "
                 "ck/cpu=%.2f ck/opaque=%.2f]", g_geom[gi].name,
                 ck_ok ? "" : "UNVERIF(", ck_med, ck_ok ? "" : ")",
                 blt_med, cpu_med,
                 ck_ok ? ck_med / cpu_med : -1.0,
                 (ck_ok && blt_med > 0) ? ck_med / blt_med : -1.0);
        } else {
            g_warn++;
            slog("[s3ckey geom=%-7s ck_MBps=UNVERIF blt_MBps=%s cpu_MBps=%.1f]",
                 g_geom[gi].name, conv >= 0 ? "(see RAW)" : "UNVERIF",
                 cpu_med > 0.0 ? cpu_med : -1.0);
        }

        if (g_geom[gi].w == 16 && g_geom[gi].h == 16) {
            cpu16 = cpu_med; blt16 = blt_med; blt16_ok = (conv >= 0);
            ck16 = ck_med; ck16_ok = ck_ok;
        }
    }
    ...
    /* ---- self-test + GATE ---- */
    slog("");
    slog("S3CKEY-SELFTEST warnings=%d", g_warn);

    slog("");
    slog("GATE (K4 colorkey -- judged AT 16x16, the sprite-tile size):");
    slog("  HARD PREREQ: colorkey verify OK on a key-reg rung (else CK_UNVERIF).");
    slog("  Question: does the ViRGE colorkey BLT keep v6's opaque ~3.7x-CPU edge,");
    slog("  or does source transparency tank it? Judged on ck/cpu @16x16.");
    slog("    GREEN_CK_KEEPS_EDGE    ck16 verified AND ck16>cpu16 AND ck16/cpu16 >= 3.0");
    slog("    AMBER_CK_REDUCED_EDGE  ck16>cpu16 but ck16/cpu16 < 3.0 (beats CPU, edge shrunk)");
    slog("    RED_CK_LOSES_TO_CPU    ck16 verified but <= cpu16 (transparency tanked it)");
    slog("    CK_UNVERIF             colorkey didn't verify (TP=bit9 mono-only? -> iter)");

    const char *verdict;
    if (!blt16_ok && !base_verified)        verdict = "RED_MMIO_UNVERIFIED";
    else if (!blt16_ok)                     verdict = "RED_OPAQUE_BLT_VERIFY_FAIL";
    else if (!ck16_ok)                      verdict = "CK_UNVERIF";
    else if (cpu16 <= 0.0)                  verdict = "CK_UNVERIF_NO_CPU_BASELINE";
    else if (ck16 <= cpu16)                 verdict = "RED_CK_LOSES_TO_CPU";
    else if (ck16 / cpu16 >= 3.0)           verdict = "GREEN_CK_KEEPS_EDGE";
    else                                    verdict = "AMBER_CK_REDUCED_EDGE";

    double ck_cpu = (ck16_ok && cpu16 > 0.0) ? ck16 / cpu16 : -1.0;
    double ck_op  = (ck16_ok && blt16 > 0.0) ? ck16 / blt16 : -1.0;
    slog("[s3ckey GATE_16x16 ck_MBps=%.1f opaque_MBps=%.1f cpu_MBps=%.1f "
         "ck/cpu=%.2f ck/opaque=%.2f verdict=%s]",
         ck16, blt16, cpu16, ck_cpu, ck_op, verdict);
    slog("  GREEN/AMBER -> colorkey BLT beats the CPU at the tile size -> the S3");
    slog("    _blit_indexed colorkey offload (Lever 2) is viable (GREEN keeps the");
    slog("    full edge; AMBER a reduced edge -- flush-instr weighs it vs integration");
    slog("    cost). RED/CK_UNVERIF -> colorkey via the 2D engine is not a win (or not");
    slog("    reachable); the opaque-only s3blt v6 result stands + colorkey stays on");
    slog("    the CPU. flush-instr applies the final S3-2 gate (with s3vram).");

    slog("");
    slog("[s3ckey SUITE_DONE]");
    slog("S3CKEY-DONE");
    if (g_log) fclose(g_log);
    return 0;
}
