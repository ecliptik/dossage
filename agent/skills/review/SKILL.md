---
description: "Reviewing a patch destined for shared/patches/ (or a port's own patches/<engine>/) against this hub's own landing conventions before it's committed -- provenance verification, DJGPP hard constraints, neutral naming, numbering/slot discipline, and the \"read gating code, not commit titles\" rule. Use this whenever a patch (yours, a peer session's, or a teammate's) is about to be committed anywhere in this hub or a port repo, especially one authored or handed over by a different session.\n"
---
...
Is the `NNNN` the actual next-free slot in `shared/patches/<vendor>/`
(`ls | sort | tail`), not a guessed or stale number? Slots are reserved,
never renumbered.
...
still have one line that regresses it.

## 4. Check neutral naming (`shared/` only)
...
A temporary patch (gated, logged, or otherwise not meant to ship
permanently) needs its removal either landed alongside it once the
