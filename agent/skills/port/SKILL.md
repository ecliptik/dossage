---
description: "Bootstrapping a new DOS port from a ports.yaml BACKLOG candidate all the way to \"a real, wired-up repo with a porting agent ready to start the compile slice\" -- upstream/license verification, scripts/new-port.sh's mechanical scaffold, self-contained skill install (both this hub's and vcctrl's skills via npx skills) shared/agents charter instantiation, vendor/sources.manifest pins, the ports.yaml claim commit, and a concrete handoff message to the port repo's own agent. Use this whenever claiming a new port candidate, starting a new port repo, or asked to \"start/bootstrap/scaffold a new port\" -- run it from this hub repo, not from inside the new port repo.\n"
---
