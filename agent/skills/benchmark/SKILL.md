---
description: "Running or recording a real-hardware performance KPI campaign for a DOS port -- writing a falsifiable KPI, picking team shape, and following the investigation/coordination discipline a real campaign converged on. Use this whenever asked to benchmark, optimize toward a target fps, run a real-hardware performance campaign, or record a benchmark result -- not for a single one-off sanity check (see dos-hardware-validation for that) or for correctness/compatibility testing (see dos-realhw- verification).\n"
---
